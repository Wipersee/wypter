...
def dashboard(request):
	if request.method == "POST":
		if 'btn_extend' in request.POST:
			...
			if extend_form.is_valid():
				...
				if user_wallet.balance - extend_form.cleaned_data['price'] < 0 :
					...
					user_wallet.balance = Decimal(user_wallet.balance) - Decimal(extend_form.cleaned_data['price'])
					user_wallet.save()
			...
			if income_form.is_valid():
				user_wallet = Wallet.objects.get(user=Profile.objects.get(user=request.user))
				user_wallet.balance = Decimal(user_wallet.balance) + Decimal(income_form.cleaned_data['price'])
				user_wallet.save()
				inc = Income.objects.create(price=income_form.cleaned_data['price'],
											wallet=user_wallet)
				inc.save()
				return redirect('/')
	else:
		...
		income_form = AddIncomeForm()
		user_wallet = Wallet.objects.get(user=Profile.objects.get(user=request.user))
		sum_extends = [x.price for x in Extend.objects.filter(wallet=user_wallet)]
